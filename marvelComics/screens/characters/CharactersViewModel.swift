...
class CharactersViewModel: ObservableObject{
    ...
    func getCharactersList(comicId: String){
        AFHttp.get(url: AFHttp.API_COMIC_SINGLE + String(comicId) + AFHttp.API_CHARACTERS_LIST, offset: 0){data in
            ...
            self.saveCharacters(characters: characters?.data?.results ?? [Character](),comicId: comicId)
        }
    }
    
    func saveCharacters(characters: [Character], comicId: String){
        let context = PersistenceController.shared.container.viewContext
        for character in characters {
            let characterEntity = Characters(context: context)
            characterEntity.comicId = comicId
            characterEntity.id = String(character.id ?? 0)
